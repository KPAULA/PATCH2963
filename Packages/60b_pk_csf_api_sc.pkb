  ...
  if pk_csf.fkg_nf_inutiliza(en_empresa_id => est_row_Nota_Fiscal.empresa_id
     ...
     if nvl(gn_tipo_integr, 0) = 1 then
        ...
        pkb_excluir_dados_nf( en_notafiscal_id => est_row_Nota_Fiscal.id
                            , ev_rotina_orig   => 'PK_CSF_API_SC.PKB_INTEGR_NOTA_FISCAL' );
        --
     end if;
     --
  end if;
...
BEGIN
  ...
  --
     vn_fase := 90;
  --
     gv_mensagem_log := 'Executada a rotina PK_CSF_API_SC.PKB_EXCLUIR_DADOS_NF para exclusao do documento fiscal ID: '||en_notafiscal_id||'. '||
