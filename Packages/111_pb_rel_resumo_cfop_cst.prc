create or replace procedure csf_own.pb_rel_resumo_cfop_cst(en_empresa_id  in empresa.id%type,
                                                           ...
                                                           en_codst_id    in cod_st.id%type, -- nulo quando não informado pela tela
                                                           en_cfop_id     in cfop.id%type, -- nulo quando não informado pela tela
                                                           ed_dt_ini      in date,
                                                           ed_dt_fin      in date,
                                                           en_consol_empr in number default 0) is
  /*
  Procedimento monta o relatório de resumo por CFOP e CST, considerando:
  ...
  Foram incluídas as colunas vl_bc_isenta_nt e vl_bc_outra na procedure pkb_seta_valores no momento em
  ...
  Excluir os dados da tabela rel_resumo_cfop_cst através do usuário (parâmetro de entrada).
  --
          ...
          for rec_inf in c_inf(en_notafiscal_id => rec.id) loop
            ...
            if nvl(vn_codst_id, 0) = nvl(en_codst_id, nvl(vn_codst_id, 0)) then
              ...
              pkb_seta_valores(en_empresa_id       => rec_emp.empresa_id,
                               ...
                               en_codst_id         => nvl(vn_codst_id, 1), -- Passa "1" para não dar erro no índice do vetor
