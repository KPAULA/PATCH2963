...
BEGIN
    ...
    /*IF nvl(en_ret_carac_espec, 0) = 1 THEN
       --vv_valor2 := nvl(ltrim(rtrim(translate(ev_string, 'ÇçåÅâÂãÃáÁàÀäÄêÊéÉèÈëËîÎíÍìÌïÏôÔÕõóÓòÒöÖûÛúÚùÙüÜÿŸıİñÑĞ\ˆ“&¤¶¦æÆø×ƒØ¿®±¾½¼«»ğßµşŞ¯÷œ¸·!¨*+=_{}[];|<>?ªº°§¹²³™š£¢¬´`~^¿¢€¥‰†‡„‹›¦ ¡•§·º…–—''’'
       vv_valor2 := nvl(ltrim(rtrim(translate(ev_string, 'ÇçåÅâÂãÃáÁàÀäÄêÊéÉèÈëËîÎíÍìÌïÏôÔÕõóÓòÒöÖûÛúÚùÙüÜÿŸıİñÑĞ\ˆ“&¤¶¦æÆø×ƒØ¿®±¾½¼«»ğßµşŞ¯÷œ¸·!¨*_{}[];|<>?ªº°§¹²³™š£¢¬´`~^¿¢€¥‰†‡„‹›¦ ¡•§·º…–—''’'
    ...
    IF nvl(en_ret_carac_espec, 0) = 1 THEN
       ...
       --
       vv_valor2 := nvl(ltrim(rtrim(translate(vv_valor2, 'ÇçåÅâÂãÃáÁàÀäÄêÊéÉèÈëËîÎíÍìÌïÏôÔÕõóÓòÒöÖûÛúÚùÙüÜÿŸıİñÑĞ\ˆ“&¤¶¦æÆø×ƒØ¿®±¾½¼«»ğßµşŞ¯÷œ¸·!¨*{}[];|<>?ªº°§¹²³™š£¢¬´`~^¿¢€¥‰†‡„‹›¦ ¡•§·º…–—''’'
                                                       , 'CcaAaAaAaAaAaAeEeEeEeEiIiIiIiIoOOooOoOoOuUuUuUuUyYyYnND '))), ' ');
       --
    ELSIF nvl(en_ret_carac_espec, 0) = 2 THEN -- NF-e
     ...
     ELSIF nvl(en_ret_carac_espec, 0) = 3 THEN -- 
       ...
       --
       vv_valor2 := nvl(ltrim(rtrim(translate(vv_valor2, 'ÇçåÅâÂãÃáÁàÀäÄêÊéÉèÈëËîÎíÍìÌïÏôÔÕõóÓòÒöÖûÛúÚùÙüÜÿŸıİñÑĞ\ˆ“¤¶¦æÆ×ƒ¿®±¾½¼«»ğßşŞ¯÷œ¸·!¨*{}[];|?ª§¹²³™š£¢¬´`~^¿¢€¥‰†‡„‹›¦ ¡•§·…–—''’'
                                                       , 'CcaAaAaAaAaAaAeEeEeEeEiIiIiIiIoOOooOoOoOuUuUuUuUyYyYnND '))), ' ');
       --
...
-------------------------------------------------------------------------------------------------------
FUNCTION fkg_converte_p ( en_empresa_id      in Empresa.id%TYPE
                      , ev_string            IN varchar2
                      , en_espacamento       IN number DEFAULT 0
                      , en_remove_spc_extra  IN number DEFAULT 1
                      , en_ret_carac_espec   IN number DEFAULT 1 -- 1-padrão,  2-NF-e, 3-(º > < " Ø µ &), 4-Mantem & (E comercial)
                      , en_ret_tecla         in number default 1 -- retira comandos CHR
...
BEGIN
    ...
    --
     select pgs.vlr_param
      ...
      where pgs.param_name = 'NF_RET_CARAC_ESPEC'
       and pgs.modulo_id in (select id from modulo_sistema where cod_modulo = 'EMISSAO_DOC')
       and pgs.grupo_id  in (select id from grupo_sistema where cod_grupo = 'VALIDACAO')
    ...
    --
    if nvl(vn_vlr_param,0) = 0
      then 
        vn_vlr_param := en_ret_carac_espec;
     end if;
    --
    -- Remove os caracteres especiais.
    /*IF nvl(en_ret_carac_espec, 0) = 1 THEN
    ...
    IF nvl(vn_vlr_param, 0) = 1 THEN
       ...
       vv_valor2 := nvl(ltrim(rtrim(translate(vv_valor2, 'ÇçåÅâÂãÃáÁàÀäÄêÊéÉèÈëËîÎíÍìÌïÏôÔÕõóÓòÒöÖûÛúÚùÙüÜÿŸıİñÑĞ\ˆ“&¤¶¦æÆø×ƒØ¿®±¾½¼«»ğßµşŞ¯÷œ¸·!¨*{}[];|<>?ªº°§¹²³™š£¢¬´`~^¿¢€¥‰†‡„‹›¦ ¡•§·º…–—''’'
                                                       , 'CcaAaAaAaAaAaAeEeEeEeEiIiIiIiIoOOooOoOoOuUuUuUuUyYyYnND '))), ' ');
       --
    ELSIF nvl(vn_vlr_param, 0) = 2 THEN -- NF-e
       vv_valor2 := nvl(ev_string, ' ');
       -- Converte o caractere especial \n "New line" por chr(10) "Enter"  
       if instr(vv_valor2, '\n') > 0 then
          vb_carac_espec := true;
       ...
       --
        vv_valor2 := nvl(ltrim(rtrim(translate(vv_valor2, 'º“&<>´`’„"''’' , ' '))), ' ');
       --
     ELSIF nvl(vn_vlr_param, 0) = 3 THEN -- 
       --vv_valor2 := nvl(ltrim(rtrim(translate(ev_string, 'ÇçåÅâÂãÃáÁàÀäÄêÊéÉèÈëËîÎíÍìÌïÏôÔÕõóÓòÒöÖûÛúÚùÙüÜÿŸıİñÑĞ\ˆ“&¤¶¦æÆø×ƒØ¿®±¾½¼«»ğßµşŞ¯÷œ¸·!¨*+=_{}[];|<>?ªº°§¹²³™š£¢¬´`~^¿¢€¥‰†‡„‹›¦ ¡•§·º…–—''’'
       --vv_valor2 := nvl(ltrim(rtrim(translate(ev_string, 'ÇçåÅâÂãÃáÁàÀäÄêÊéÉèÈëËîÎíÍìÌïÏôÔÕõóÓòÒöÖûÛúÚùÙüÜÿŸıİñÑĞ\ˆ“&¤¶¦æÆø×ƒØ¿®±¾½¼«»ğßµşŞ¯÷œ¸·!¨*_{}[];|<>?ªº°§¹²³™š£¢¬´`~^¿¢€¥‰†‡„‹›¦ ¡•§·º…–—''’'
       vv_valor2 := nvl(ev_string, ' ');
       -- Converte o caractere especial \n "New line" por chr(10) "Enter"
       ...
       vv_valor2 := nvl(ltrim(rtrim(translate(vv_valor2, 'ÇçåÅâÂãÃáÁàÀäÄêÊéÉèÈëËîÎíÍìÌïÏôÔÕõóÓòÒöÖûÛúÚùÙüÜÿŸıİñÑĞ\ˆ“¤¶¦æÆ×ƒ¿®±¾½¼«»ğßşŞ¯÷œ¸·!¨*{}[];|?ª§¹²³™š£¢¬´`~^¿¢€¥‰†‡„‹›¦ ¡•§·…–—''’'
                                                       , 'CcaAaAaAaAaAaAeEeEeEeEiIiIiIiIoOOooOoOoOuUuUuUuUyYyYnND '))), ' ');
       --
    ELSIF nvl(vn_vlr_param, 0) = 4 THEN -- Mantem & (E comercial)
       vv_valor2 := nvl(ev_string, ' ');
       -- Converte o caractere especial \n "New line" por chr(10) "Enter"
       if instr(vv_valor2, '\n') > 0 then
          vb_carac_espec := true;
  ...
  ELSE
    ...
    --
    -- Limpa caracteres Unicode
    if nvl(vn_vlr_param, 0) not in (2,3,4) then  
       vv_valor3 := ASCIISTR(vv_valor3);
       vv_valor3 := REPLACE( vv_valor3, '\0081', '');
       vv_valor3 := REPLACE( vv_valor3, '\00AD', '');
       vv_valor3 := REPLACE( vv_valor3, '\00BF', '');
...
EXCEPTION
   WHEN OTHERS THEN
      IF SQLCODE <> -20001 THEN
         raise_application_error(-20001, 'Erro na fkg_converte_p : ' || SQLERRM);
      END IF;
      RAISE;
...
BEGIN
   ...
   vi := 0;
   -- Remove os caracteres especiais.
   vv_valor2 := nvl(ltrim(rtrim(translate(ev_string, 'ÇçåÅâÂãÃáÁàÀäÄêÊéÉèÈëËîÎíÍìÌïÏôÔÕõóÓòÒöÖûÛúÚùÙüÜÿŸıİñÑĞ\ˆ“&¤¶¦æÆø×ƒØ¿®±¾½¼«»ğßµşŞ¯÷œ¸·"!#%¨*={}[]|<>:?ªº°§¹²³™š£¢¬´`~^¿¢€¥‰†‡„‹›¦ ¡•§·º…–—"''’'
                                                   , 'CcaAaAaAaAaAaAeEeEeEeEiIiIiIiIoOOooOoOoOuUuUuUuUyYyYnND'))), ' ');
   --
...
BEGIN
   ...
   vv_valor2 := nvl(ltrim(rtrim(translate(ev_string, 'ÇçåÅâÂãÃáÁàÀäÄêÊéÉèÈëËîÎíÍìÌïÏôÔÕõóÓòÒöÖûÛúÚùÙüÜÿŸıİñÑĞ\ˆ“¤¶¦æÆø×ƒ¿®±¾½¼«»ğßşŞ¯÷œ¸·¨*?ª§¹²³™š£¢¬´`~^¿¢€¥‰†‡„‹›¦ ¡•§·…–—''’'
                                                   , 'CcaAaAaAaAaAaAeEeEeEeEiIiIiIiIoOOooOoOoOuUuUuUuUyYyYnND'))), ' ');
   --
   vv_valor2 := REPLACE( vv_valor2, chr(9), '');  -- HT-Horizontal Tab
   vv_valor2 := REPLACE( vv_valor2, chr(27), ''); -- ESC-Escape
