      ...
      for i in vt_tab_csf_nota_fiscal_fisco.first..vt_tab_csf_nota_fiscal_fisco.last loop
         ...
         --
            if nvl(vn_dm_tipo,0) <> 1 then
                 --
                 pk_csf_api.gv_mensagem_log := 'O dm_tipo deve ser igual a 1 quando temos Informacões do documento de arrecadação referenciado, corrigir via Web-Service na tabela '||
                                               '<NFInfor_Adic> ou via open interface através da view <VW_CSF_NFInfor_Adic>.';
                 --							
                 declare
                    vn_loggenerico_id  log_generico_nf.id%TYPE;
                 begin
...
is
   ...
   select nf.empresa_id
    ...
    where nf.empresa_id         = en_empresa_id
      ...
      and ( nf.cod_msg is null or nf.cod_msg not in ('204', '290') )
...
begin
   ...
   vn_fase := 3;
   --
   if nvl(vt_tab_csf_cons_chave_nfe.count,0) > 0 then 
      --
      for i in vt_tab_csf_cons_chave_nfe.first..vt_tab_csf_cons_chave_nfe.last loop
         ...
         -- Se existir sai busca próximo registro da view pq a chave em questão já foi 
         -- integrada
         if nvl(pk_csf_api_cons_sit.fkg_checa_existe_chave(vt_tab_csf_cons_chave_nfe(i).nro_chave_nfe),0) = 0 then   
            --
         pk_csf_api_cons_sit.gt_row_csf_cons_sit := null;
         ...
         pk_csf_api_cons_sit.pkb_integr_cons_chave_nfe ( est_log_generico_nf  => vt_log_generico_nf
                                                       ...
                                                       , en_multorg_id        => gn_multorg_id 
                                                       , ev_rotina            => 'pk_integr_view_KARINA.pkb_ler_cons_chave_nfe' );
         --
         end if;
         --   
      end loop;
