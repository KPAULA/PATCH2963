CREATE OR REPLACE TRIGGER csf_own.T_A_U_ITEM_AGEND_INTEGR_01
...
declare
  ...
  -- Redmine #75700 - Integração Nova America
  --                - Empresa NOVA AMERICA - Foram incluidos (vn_hash not in ('c0c7c76d30bd3dcaefc96f40275bdc0a','c4ca4238a0b923820dcc509a6f75849b')
  --                - or vn_descr not in( 'VENANCIO','DEFAULT MULT-ORGANIZACAO') na validacao para a trigger nao ser acionada.
  --
...
begin
  ...
  vn_fase := 5;
  --
  -- Verifica se a hash ou a descrição pertence a Venancio ou Nova America,
  -- caso pertençam aoS mesmoS, a trigger não deve ser acionada
 if (vn_hash not in ('c0c7c76d30bd3dcaefc96f40275bdc0a','c4ca4238a0b923820dcc509a6f75849b')
   or vn_descr not in( 'VENANCIO','DEFAULT MULT-ORGANIZACAO')
   )  then
    --
    begin
     select a.dt_agend
